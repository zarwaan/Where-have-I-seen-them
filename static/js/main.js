...
// inputMedia.addEventListener('input',() => console.log("Value changed ",inputMedia.value));
let debounceTimeout;
...
return {
    ...
    "bearer-token": MY_BEARER_TOKEN
        ...
        console.log("Erro fetching API key: ",error);
...
function selectThisOption(mediaName,posterPath,overview,mediaType,id){
    console.log([mediaName,posterPath,overview,mediaType,id]);
    inputMedia.value = mediaName;
    // movieTime(null,mediaName,posterPath,mediaType,overview,id);
    // findMovie(null,mediaName,id,mediaType);
    // window.open(`/main?mediaName=${mediaName}&mediaId=${id}&mediaType=${mediaType}`,'_self'); //i was here
        ...
        try {
            ...
            if(mediaType === 'tv'){
                ...
                try {
                    ...
                    console.error("i dont even know what is wrong: ",error)
                }
            }
...
{
    // console.log(posterPath, backdropPath, title, year, runtime, language, genres, tagline, overview, directorCreators, budget, revenue)
    removeErrorMessage();
...
function renderCastList(castArg,mediaType){
    ...
    for(let i=0;i<Math.min(cast.length,15);i++){
        ...
        castCard.addEventListener('click',() => {
            // window.open(`/search?name=${encodeURIComponent(actornameurl)}&id=${encodeURIComponent(actoridurl)}`)
...
document.getElementById('search').addEventListener('click',(event)=>{
    // findMovie(event,document.getElementById('query').value)
    // window.open(`/main?mediaName=${document.getElementById('query').value}`,'_self'); // i was here
    window.open(`/media?query=${document.getElementById('query').value}`,"_self");
});

document.getElementById('search-form').addEventListener('submit',(event) => {
    // findMovie(null,document.getElementById('query').value)
    // window.open(`/main?mediaName=${document.getElementById('query').value}`,'_self'); // i was here
    window.open(`/media?query=${document.getElementById('query').value}`,"_self");
...
async function toggleWatched(mediaId,mediaType){
    ...
    try{
        ...
        if(logMediaResponse.ok){
            console.log(logMediaResult);
        }
        else{
            console.error(logMediaResult);
        }
    }
    catch(error){
...
async function setWatched(mediaId,mediaType){
    // SQL for fetching watch status with movieID and personID from db
    if(!Cookies.get('userInfo')){
        watched = false;
    }
    else{
        try{
            ...
            if(watchStatusResponse.ok){
                console.log(watchStatusResult.watched);
                watched = await watchStatusResult.watched;
                console.log(watched);
            }
            else{
                console.error(watchStatusResult);
            }
        }
        catch(error){
            console.error("server error: ",error);
...
async function fetchAllPlaylists(id,type) {
    ...
    try{
        ...
        if(checkInWatchlistResponse.ok){
            ...
            console.error(checkInWatchListResult);
        }
    }
    catch(error){
    ...
    for(const playlist of playlists){
        ...
        try{
            ...
            if(checkInPlaylistResponse.ok){
                ...
                console.error(checkInPlaylistResult);
            }
        }
        catch(error){
            console.log('Server error: ',error);
        ...
        if(playlist.checked && !playlist.disabled){
            if(playlist.dataset.whistPlaylistId==='watchlist'){
                try{
                    ...
                    console.error(addToWatchlistResult);
                }
            catch(error){
                ...
                try{
                    ...
                    if(addToPlaylistResponse.ok)
                        console.log(addToPlaylistResult);
                    else
                    console.error(addToPlaylistResult);
                }
            catch(error){
                setErrorMessage('Server error: ',error);
                }
...
// console.log(mediaNameURL, mediaIDURL, mediaTypeURL);
// if(mediaIDURL && mediaNameURL!==null)
//     findMovie(null,mediaNameURL,mediaIDURL,mediaTypeURL);
// else if(!mediaIDURL && mediaNameURL!==null)
//     findMovie(null,mediaNameURL,null,null);
